...
def _delete_organization(org_name):
	...
	if org_id != 0:
		...
		url= 'http://admin:admin@localhost:3000/api/orgs/' + str(org_id)
		headers={"Content-Type": 'application/json'}
...
def _get_global_user_id(user_login): #without switching to org
	url='http://admin:admin@localhost:3000/api/users/lookup?loginOrEmail=' + str(user_login)
	headers={"Content-Type": 'application/json'}
...
def _delete_user(user_login):
	...
	if user_id != 0:
		url = 'http://admin:admin@localhost:3000/api/admin/users/' + str(user_id)
		headers={"Content-Type": 'application/json'}
		response = requests.delete(url, headers=headers)
		print (response.text)
...
def _remove_user_from_org(user_login):
	...
	user_id = _get_global_user_id(user_login)
	if user_id != 0:
		url = 'http://admin:admin@localhost:3000/api/org/users/' + str(user_id)
		headers={"Content-Type": 'application/json'}
		response = requests.delete(url, headers=headers)
		print (response.text)
...
def _delete_datasource(datasource_name):
	url="http://admin:admin@localhost:3000/api/datasources/name/" + str(datasource_name)
	headers={"Content-Type": 'application/json'}
	response = requests.delete(url, headers=headers)
	print (response.text)
...
def _update_dashboard(name, id, datasource_name, measurement1, parameter1, measurement2, parameter2, measurement3, parameter3):
	...
	data = {
	  "dashboard": {
	    ...
	    "time": {
	      "from": "2019-11-13T11:00:11.714Z",
	      "to": "2019-11-13T11:10:16.908Z"
	    },
	    "timepicker": {
	      "refresh_intervals": [
	        "5s",
...
def _get_dashboard_uid(dash_title):
	url= "http://admin:admin@localhost:3000/api/search?folderIds=0&query=&starred=false"
	headers={"Content-Type": 'application/json'}
	response = requests.get(url, headers=headers)
	#print (response.text)
...
def _get_dashboard_json(dash_title, org):
	...
	if dash_uid != 0:
		url= "http://admin:admin@localhost:3000/api/dashboards/uid/" + str(dash_uid)
		headers={"Content-Type": 'application/json'}
		response = requests.get(url, headers=headers)
